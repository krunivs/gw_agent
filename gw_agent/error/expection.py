...
from gw_agent.error.types import ErrorType
...
class SystemInternalError(Exception):
    ...
    arg0: (ErrorType) error_type: user defined error type
...
class ErrorHandler(Exception):
    ...
    desc: write error time and redefine error types, provide printing error log, json formatted error by user-level
    """
    DEBUG_MODE = 0x01
    DEPLOY_MODE = 0x02
    ...
    def __init__(self, exception):
        ...
        self.time = time.time()
        self.__set_error_desc(exception)
        self.__set_error_type(exception)
        self.__print_error_log()


    def __print_error_log(self):
        """
        desc: print error(Exception) log to app logger channel
        :return:
        """
        str_error = \
    ...
    def __set_error_desc(self, exception):
        """
        desc: set error argument from Exception raised
        :param exception:
        :return:
        """
    ...
    def no_error():
        """
        desc: returns json formatted 'no error'
        :return:
        """
    ...
    def get_api_error(self):
        """
        desc: returns json formatted api-level error which filtered by api-level
        :return:
        """
        if self.mode == ErrorHandler.DEBUG_MODE:
            ...
            if type(self.error) is APIError:
                ...
                return {'time': self.time,
                        'code': self.error_type.value['code'],
                        'text': 'system internal error',
                        'desc': 'contact admin with error code and time'}


    def get_system_error(self):
        """
        desc: returns json formatted system-level error which no filtered with traceback information
        :return:
        """
        return {'time': self.time,
                'code': self.error_type.value['code'],
    ...
    def __set_error_type(self, exception):
        """
        desc: set error types from ErrorType enum class
        :param exception:
        :return:
        """
